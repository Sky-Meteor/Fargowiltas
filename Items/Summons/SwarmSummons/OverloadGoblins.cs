    ...
    public class OverloadGoblins : ModItem
    {
        ...
        public override bool? UseItem(Player player)
        {
            if (FargoWorld.OverloadGoblins)
            {
                ...
                if (Main.netMode == 2)
                {
                    ChatHelper.BroadcastChatMessage(NetworkText.FromLiteral(FargoUtils.IsChinese() ? "哥布林们冷静下来了！" : "The goblins have calmed down!"), new Color(175, 75, 255));
                }
                else
                {
                    Main.NewText(FargoUtils.IsChinese() ? "哥布林们冷静下来了！" : "The goblins have calmed down!", 175, 75, 255);
                }
            }
            else
            {
