    ...
    public class OverloadGoblins : ModItem
    {
        ...
        public override bool? UseItem(Player player)
        {
            if (FargoWorld.OverloadGoblins)
            {
                ...
                if (Main.netMode == 2)
                {
                    ChatHelper.BroadcastChatMessage(NetworkText.FromLiteral(Language.GetTextValue("Mods.Fargowiltas.MessageInfo.OverloadGoblins")), new Color(175, 75, 255));
                }
                else
                {
                    Main.NewText(Language.GetTextValue("Mods.Fargowiltas.MessageInfo.OverloadGoblins"), 175, 75, 255);
                }
            }
            else
            {
